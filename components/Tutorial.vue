  ...
  >
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="mt-8 bg-white overflow-hidden shadow sm:rounded-lg p-6">
        <h2 v-if="$auth.loggedIn" class="text-2xl leading-7 font-semibold">
          Welcome,
          {{
        ...
        <h2 v-else class="text-2xl leading-7 font-semibold">
          Welcome to your Nuxt Application
        </h2>
        <p v-if="$auth.loggedIn" class="mt-3 text-gray-600">
          <span v-if="$auth.user.email !== undefined">Your email is <b>{{ $auth.user.email }}</b></span>
        </p>
        ...
        <p
          v-if="$auth.loggedIn"
          class="mt-4 pt-4 text-gray-800 border-t border-dashed text-xs"
        >
          User ID: {{ $auth.user.sub }}
        </p>
        <p v-else class="mt-4 pt-4 text-gray-800 border-t border-dashed">
